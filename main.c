...
void Sign(SignArgs* args) {
    ...
    // Seek to the specified offset before beginning hashing
    ...
    while ((readBytes = fread(buffer, 1, PAGE_SIZE, args->dataFile)) > 0) {
        ...
        SHA256_Init(&shaCtx);
        // Always hash the entire PAGE_SIZE (4096 bytes) even for partial pages
        // This ensures compatibility with professional tools that expect the same behavior
        ...
        // Clear the buffer completely before the next read
...
int main(int argc, char* argv[]) {
    ...
    if (argc >= 4) {
        textOffset = strtoul(argv[3], NULL, 0); // Accepts both 0x1000 and 4096 formats
    }
    ...
    FILE* dataFile = fopen(appPath, "rb");  // Ensure we're using binary mode
    if (dataFile == NULL) {
    ...
    // If text size wasn't specified, use the whole file size minus offset
    if (args.textSize == 0) {
